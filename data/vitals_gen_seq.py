...
data['for_training'] = data['Time'].apply(lambda x: int(difference_time_A_B_seconds(timeA=test_time,timeB=x,format_pattern="%m/%d/%Y %H:%M")>0))
...
training_impute=data[data['for_training']==1]._get_numeric_data().median() # only use the training data to compute median and impute. use _get_numeric_data() or the mean/median computation will be super slow due to the non-numerical columns!!!
print(training_impute)

print('mean value computed')


features_to_be_normalized_list=['SBP','DBP','Pulse','Temp','SpO2','Resp','age'] # we will save and use this in the training process later
mean_std_dic={}
...
for feature in features_to_be_normalized_list:
  mean_std_dic[feature]={'mean':training_mean[feature],
...
for i,PID in enumerate(all_PID):
  ...
  for adm_index in adm_index_list:
    ...
    data_PID=copy.deepcopy(data_PID_all_adm_index[data_PID_all_adm_index['adm_index']==adm_index])
    data_PID=data_PID.reset_index(drop=True) # reset the index, or they could be inconsistent due to the previous overlapping
    
    #print(data_PID)
  
    for ind in data_PID.index:
      #if ind+period_num-1 not in data_PID.index:
      if ind+2-1 not in data_PID.index: # in v5.5 we accept shorter seqs, as short as 2 periods
        continue
      
      if data_PID.loc[ind,'adm_start']==1: # v5.5: seq len at most 12
        
        
        seq=copy.deepcopy(data_PID.loc[ind:ind+period_num-1]) # pandas .loc index includes both bounds!
        
        #if seq.isna().sum().sum()/(period_num*feature_number)>0.05:
        if seq.isna().sum().sum()/(seq.shape[0]*feature_number)>0.2: # use 0.1 for 4h gap to get more samples, also compute the missing ratio in this correct way
          continue
        ...
        data_new.append(seq)
        #data_new=pd.concat([data_new, seq])
