...
die_eventually_66h_IncludingHome_dic = (
    {}
)  # for each unique adm_index, record if the patient die eventually after 72h since relaxed adm
...
for i, PID in enumerate(all_PID):
    ...
    all_time = (
        []
    )  # collect all vitals time, so later use the earliest one as the initial time of the seq

    all_adm_time = [(t["time"], t["time_end"]) for t in data_dic[PID]["adm_records"]]
    all_adm_time = list(set(all_adm_time))

    adm_time_drg_dic = {
        g: [] for g in all_adm_time
    }  # since one adm can have multiple drg codes, we use a list to record all of them!!!!

    for t in data_dic[PID]["adm_records"]:
        adm_time_drg_dic[(t["time"], t["time_end"])].append(t["drg_code"])

    all_adm_time.sort(
        key=lambda date: datetime.strptime(date[0], "%m/%d/%Y %H:%M")
    )  # sort the (adm_start,adm_end) pairs by the adm_start time
    ...
    for time, time_end in all_adm_time:
                ...
                if (
                    abs(
                        difference_time_A_B_seconds(
                            timeA=time,
                            timeB=covid_pos_time,
                            format_pattern="%m/%d/%Y %H:%M",
                        )
                    )
                    <= 7 * 24 * 3600
                ):  # as long as there is one positive covid test in adm_time+=7 days, regarding this adm as covid adm
        ...
        relaxed_adm_time = A_plus_hrs(
            timeA=time, hr=-time_relaxation_hr, format_pattern="%m/%d/%Y %H:%M"
        )
        ...
        for feature in feature_dic_empty:
            ...
            if (
                ...
                + demo_feature_list
                + pmh_feature_list
            ):  # these are not in database pkl but will be recorded later
                continue
            ...
            if feature not in [
                ...
                for value_dic in data_dic[PID][feature]:
                    ...
                    if (
                        difference_time_A_B_seconds(
                            timeA=value_dic["time"],
                            timeB=first_recorded_time,
                            format_pattern="%m/%d/%Y %H:%M",
                        )
                        >= 0
                        ...
                        <= 0
                    ):  # make sure that the feature is within relaxed adm duration
                        ...
                        our_period_stamp = A_plus_hrs(
                            timeA=first_recorded_time,
                            hr=k * tau,
                            format_pattern="%m/%d/%Y %H:%M",
                        )

                        if our_period_stamp not in features_wrt_gaps:
                            features_wrt_gaps[our_period_stamp] = copy.deepcopy(
                                feature_dic_empty
                            )

                        features_wrt_gaps[our_period_stamp][feature].append(
                            value_dic["value"]
                        )
                        ...
                        for demo_feature in demo_feature_list:
                            ...
                            if demo_feature == "age":
                                age = difference_time_A_B_seconds(
                                    timeA=our_period_stamp,
                                    timeB=demo_data_dic[PID]["birthday"],
                                    format_pattern="%m/%d/%Y %H:%M",
                                ) / (365.25 * 24 * 3600)
                                features_wrt_gaps[our_period_stamp][
                                    demo_feature
                                ].append(age)
                            else:
                                features_wrt_gaps[our_period_stamp][
                                    demo_feature
                                ].append(demo_data_dic[PID][demo_feature])
                        ...
                        for pmh_feature in pmh_feature_list:
                            ...
                            if (
                                difference_time_A_B_seconds(
                                    timeA=pmh_data_dic[PID][pmh_feature],
                                    timeB=our_period_stamp,
                                    format_pattern="%m/%d/%Y %H:%M",
                                )
                                < 0
                            ):  # pmh before the current time stamp

                                features_wrt_gaps[our_period_stamp][pmh_feature].append(
                                    1
                                )

                        # -----------------------------------------

                        features_wrt_gaps[our_period_stamp]["adm_index"].append(
                            adm_index
                        )  # record the unique adm index for each adm span
                        features_wrt_gaps[our_period_stamp]["drg_code"].append(drg_code)

            else:

                all_outcome_start_time = (
                    []
                )  # later use this to compute mean/median gaps!
                ...
                for value_dic in data_dic[PID][feature]:
                    ...
                    if (
                        ...
                        >= 0
                        and difference_time_A_B_seconds(
                            timeA=value_dic["time_end"],
                            timeB=time_end,
                            format_pattern="%m/%d/%Y %H:%M",
                        )
                        <= 24 * 3600
                    ):  # make sure that the feature is within relaxed adm duration. for icu and intu, maybe the time is before adm, so we also add them into relaxation  # allow a delay of end records
                        ...
                        for ii, num_tau in enumerate(
                            range(k, k_end + 1)
                        ):  # for death, end+1 gives only the beginning period. give every period in a begin/discharge duration the positive label
                            ...
                            features_wrt_gaps[our_period_stamp][
                                ...
                                value_dic["time"]
                            )  # record the adm/icu/intu/death start time

                            if ii == 0 and feature == "adm_records":
                                features_wrt_gaps[our_period_stamp]["adm_start"].append(
                                    1
                                )  # an indicator that there is at least one adm start point in this period
                            ...
                            features_wrt_gaps[our_period_stamp]["drg_code"].append(
                                drg_code
                            )

                if all_outcome_start_time != []:
                    all_outcome_start_time.sort(
                        key=lambda date: datetime.strptime(date, "%m/%d/%Y %H:%M")
                    )
                    first_outcome_start_time = all_outcome_start_time[0]
                    outcome_gap = (
                        difference_time_A_B_seconds(
                            timeA=first_outcome_start_time,
                            timeB=time,
                            format_pattern="%m/%d/%Y %H:%M",
                        )
                        / 3600
                    ...
                    print(
                        PID,
                        (time, time_end),
                        first_outcome_start_time,
                        feature,
                        outcome_gap,
                    )

                    outcome_gaps[feature].append(
                        [PID, (time, time_end), first_outcome_start_time, outcome_gap]
                    )
                    ...
                    if (
                        feature == "death_records"
                    ):  # 66h: 12 periods starting from adm-6h
            ...
            if feature not in [
                ...
                if features_wrt_gaps[our_period_stamp][feature] == []:
                    if feature in feature_type["continuous"]:
                        features_wrt_gaps[our_period_stamp][
                            feature
                        ] = ""  # -99.0 # if no certain records in a period, record it as missing using -99
                    else:
                        features_wrt_gaps[our_period_stamp][feature] = 0
                else:
                    if feature in feature_type["continuous"]:
                        features_wrt_gaps[our_period_stamp][feature] = np.mean(
                            features_wrt_gaps[our_period_stamp][feature]
                        )  # else, using the average of those values
                    else:
                        features_wrt_gaps[our_period_stamp][feature] = np.max(
                            features_wrt_gaps[our_period_stamp][feature]
                        )
                ...
                if features_wrt_gaps[our_period_stamp][feature] == []:
                    ...
                    features_wrt_gaps[our_period_stamp][feature] = features_wrt_gaps[
                        our_period_stamp
                    ][feature][
                        0
                    ]  # just use the first recorded time
...
csv_write.writerow(
    ...
    + feature_in_head
    + [
        "eventually_die_after_66h_IncludingHome",
        "eventually_die_after_66h_StrictDischarge",
        "eventually_die_all_IncludingHome",
        "eventually_die_all_StrictDischarge",
        "eventually_die_within3weeks_IncludingHome",
        "is_covid_adm",
    ]
)
...
for PID in sorted(list(averaged_data_dic.keys())):
    ...
    for our_period_stamp in all_our_period_stamp:
        ...
        if (
            int(averaged_data_dic[PID][our_period_stamp]["adm_index"])
            in die_eventually_66h_IncludingHome_dic
        ):
        ...
        if (
            int(averaged_data_dic[PID][our_period_stamp]["adm_index"])
            in die_eventually_66h_StrictDischarge_dic
        ):
        ...
        if (
            int(averaged_data_dic[PID][our_period_stamp]["adm_index"])
            in die_eventually_all_IncludingHome_dic
        ):
        ...
        if (
            int(averaged_data_dic[PID][our_period_stamp]["adm_index"])
            in die_eventually_all_StrictDischarge_dic
        ):
        ...
        if (
            int(averaged_data_dic[PID][our_period_stamp]["adm_index"])
            in die_eventually_within3weeks_IncludingHome_dic
        ):
        ...
        out_line = (
            [PID, our_period_stamp]
            + [
                averaged_data_dic[PID][our_period_stamp][feature]
                for feature in feature_in_head
            ]
            + [
                eventually_die_after_66h_IncludingHome,
                eventually_die_after_66h_StrictDischarge,
