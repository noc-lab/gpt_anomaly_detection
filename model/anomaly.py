...
from transformers.models.bert.modeling_bert import (
    ...
    BertEncoder,
    BertPooler,
)  # NOTE: the BERT (or other modules apart from GPT) was used during the development process and partially used as placeholders in this script, and will not be actually loaded/trained
...
our_config = {
    ...
    "msa_num": int(
        sys.argv[10]
    ),  # how many msas (apart from original seq) shall be used at most
    "neighbor_max_distance": float(
        sys.argv[11]
    ),  # set a constraint for the max distance from original seq to be a msa seq in version V3
    "mlm": False,
    "mlm_dup_factor": 9,
...
if our_config["mode"] == "train":  # only generating csv during training
    ...
    out = open(
        mission_name + ".csv", "a", newline="", encoding="utf-8"
...
all_features = {
    ...
    "outcome": [
        "icu_records",  # these features will be predicted, and will be always masked in input so they provide no information
        "intu_records",
        "death_records",
    ],
}
...
no_dot_all_features = {
    k: [f.replace(".", "_") for f in all_features[k]] for k in all_features
}
...
normalization_factor_dic = pickle.load(
    open("../mean_std_dic.pkl", "rb")
)  # in v6, use standardization


def extract_dataset_from_df_for_mlm(
    df, sample_ind_list, mlm=True, max_seq_len=6, interval=None
):
    ...
    for sample_ind in sample_ind_list:
        ...
        for k in normalization_factor_dic:
            df_sample_ind_ori_normalized[k] = (
                df_sample_ind_ori_normalized[k] - normalization_factor_dic[k]["mean"]
            ) / normalization_factor_dic[k][
                "std"
            ]  # in v6, use standardization
        ...
        sample_dic["Time"] = df_sample_ind.loc[
            df_sample_ind.index[0], "Time"
        ]  # use the first time stamp in seq
        sample_dic["for_training"] = df_sample_ind.loc[
            df_sample_ind.index[0], "for_training"
        ]
        ...
        sample_dic["df_sample_ind_tokenid"] = copy.deepcopy(
            df_sample_ind_ori_normalized
        )

        sample_dic["eventually_die_after_66h_IncludingHome"] = df_sample_ind.loc[
            df_sample_ind.index[0], "eventually_die_after_66h_IncludingHome"
        ]
        sample_dic["eventually_die_after_66h_StrictDischarge"] = df_sample_ind.loc[
            df_sample_ind.index[0], "eventually_die_after_66h_StrictDischarge"
        ]
        sample_dic["eventually_die_all_IncludingHome"] = df_sample_ind.loc[
            df_sample_ind.index[0], "eventually_die_all_IncludingHome"
        ]
        sample_dic["eventually_die_all_StrictDischarge"] = df_sample_ind.loc[
            df_sample_ind.index[0], "eventually_die_all_StrictDischarge"
        ]
        sample_dic["eventually_die_within3weeks_IncludingHome"] = df_sample_ind.loc[
            df_sample_ind.index[0], "eventually_die_within3weeks_IncludingHome"
        ]

        sample_dic["is_covid_adm"] = df_sample_ind.loc[
            df_sample_ind.index[0], "is_covid_adm"
        ]  # add covid adm indicator in v6.2

        sample_dic["adm_index"] = df_sample_ind.loc[
            df_sample_ind.index[0], "adm_index"
        ]  # record the adm_index in v7, for later case study
...
def pad_seq(seq, max_seq_len):
    ...
    if seq.shape[0] > max_seq_len:
        raise ValueError("wrong data, please check")
...
def mask_one_sample_causal(sample_dic, position=None):
    ...
    ori_normalized_left_shifted.loc[ori_normalized_left_shifted.index[-1] + 1, :] = (
        ori_normalized.loc[ori_normalized.index[-1], :]
    )
    ori_normalized_left_shifted = ori_normalized_left_shifted.loc[
        ori_normalized_left_shifted.index[1:], :
    ]
    ...
    masked_tokenid[all_features["outcome"]] = (
        0  # now we mask outcomes as 0 in input so they are not informative at all
    )

    mask = masked_tokenid * 0
    mask.loc[mask.index[: sample_dic["seq_len"] - 1], :] = (
        1  # in v5.5, include short seqs. if seq_len=7, we only care about the first 6 prediction, since we have no groundtruth for 7th
    )
    mask[all_features["informational"]] = (
        0  # these informational features like demo will not contribute to the loss
    )

    return (
        masked_tokenid[all_features["vitals"]].values,
...
if our_config["mode"] == "data":
    ...
    data = extract_dataset_from_df_for_mlm(
        df=df_all,
        sample_ind_list=all_sample_ind_list,
        mlm=True,
    ...
    raise ValueError("Data generation finished")
...
def assert_drg(sample):
    ...
    drg_exclude_dic = {
        ...
        "None": [
            -99
        ],  # "None" and -99 indicate a missing drg in our dataset. 61904 cases have non-missing drg
        "HCFA": [],
        "APR-DRG V30": [
            10,
            20,
...
test = [
    i for i in data if i["for_training"] == 0
]  # data is really limited, we don't set val set
...
class our_bert(
    BertModel
):  # NOTE: the BERT here was used during the development process and partially used as placeholders in this script, which will not be actually loaded/trained
    def __init__(self, config):
        super().__init__(config)

        causal_encoder_layer = torch.nn.TransformerEncoderLayer(
        ...
        self.causal_encoder = torch.nn.TransformerEncoder(
            encoder_layer=causal_encoder_layer,
        ...
        self.vitals_emb_dic = torch.nn.ModuleDict(
            {
                name: torch.nn.Embedding(
                    our_config["vitals"]["emb_size"], our_config["vitals"]["emb_dim"]
                )
                for name in no_dot_all_features["vitals"]
            }
        )

        self.embed_map = torch.nn.Linear(
            len(all_features["vitals"]), config.hidden_size
        )  # now we don't use tokenization!
        ...
        self.seq_death_classification_head = torch.nn.Linear(
            config.hidden_size, 2, bias=True
        )  # use the last hidden state to do seq classification
    ...
    def forward(
        ...
        inputs_embeds = self.embed_map(
            inputs_embeds
        )  # now we directly use input vectors
        ...
        sequence_output = self.causal_encoder(
        ...
        seq_repr_vector = torch.stack(
            [sequence_output[i, seq_len[i] - 2, :] for i in range(seq_len.shape[0])], 0
        )  # in v5.5, use the last valid hidden state
        seq_death_classification_pred = self.seq_death_classification_head(
            seq_repr_vector
        )

        death_prob = torch.nn.functional.softmax(seq_death_classification_pred, dim=1)[
            :, 1
        ]

        regression_pred = [
            self.regression_head_dic[no_dot_all_features["vitals"][num]](
                sequence_output
            )
            for num in range(len(all_features["vitals"]))
        ]
        ...
        if ori_normalized is not None:
            ...
            if self.training:
                ...
                seq_death_classification_loss = seq_classification_ce_loss(
                    seq_death_classification_pred.view(-1, 2), eventually_die.view(-1)
                )
                ...
                for i, name in enumerate(all_features["vitals"]):
                    if name not in all_features["categ"]:
                        mixed_loss.append(
                            mae_loss(
                                regression_pred[i][:, :, 0], ori_normalized[:, :, i]
                            )
                        )
                        ...
                        mixed_loss.append(
                            ce_loss(
                                regression_pred[i].view(-1, 2),
                                ori_normalized[:, :, i]
                                .view(-1)
                                .type(torch.LongTensor)
                                .cuda(),
                            ).view(ori_normalized.shape[0], ori_normalized.shape[1])
                        )

                mixed_loss = torch.stack(tuple(mixed_loss), -1)

            else:
                seq_classification_ce_loss = torch.nn.CrossEntropyLoss(
            ...
            all_loss = torch.sum(mixed_loss, dim=[-2]) / torch.sum(
                mask, dim=[-2]
            )  # only average across periods, so we save loss for each feature!

            all_loss = torch.cat(
                [all_loss, seq_death_classification_loss.reshape(-1, 1)], dim=1
            )  # IMPORTANT:   the last dim of loss is the seq classification loss!!!
            ...
            loss = torch.sum(mixed_loss) / torch.sum(mask) + torch.mean(
                seq_death_classification_loss
            )  # this is for optimization, later we may think about if we should direcetly average over samples rather than tokens, in case that there is mask imbalance in future
        ...
        return (
            ...
            all_period_loss,
            death_prob,
        )  # , regression_output_pred, repr_out


def pred_one_dataset_batch_causal(
    model, dataset, batchsize=our_config["eval_batchsize"], ignore_index=-100
):
    ...
    for r in range(int(len(dataset) / batchsize) + 1):  # +1: fix the previous bug
        ...
        masked = [
            mask_one_sample_causal(sample_dic=i) for i in sample_batch
        ]  # for causal training, we don't use [mask] token, but directly predict the next period features!

        masked_tokenid = torch.Tensor(
            [i[0] for i in masked]
        ).cuda()  # now they are real values
        mask = torch.LongTensor(
            [i[1] for i in masked]
        ).cuda()  # do not mask the last period since we are predicting nothing
        ori_normalized_left_shifted = torch.Tensor(
            [i[2] for i in masked]
        ).cuda()  # left-shift the original features, so the current-period hidden state is actually predicting the next period features!

        eventually_die = torch.LongTensor(
            [i[3] for i in masked]
        ).cuda()  # death label for seq classification
        seq_len = torch.LongTensor([i[4] for i in masked]).cuda()  # record the seq len

        loss, all_loss, all_period_loss, death_prob = model(
            inputs_embeds=masked_tokenid,
            mask=mask,
...
if our_config["ckpt"]:
    model = our_bert.from_pretrained(
        our_config["ckpt"]
    ).cuda()  # NOTE: the BERT (or other modules apart from GPT) was used during the development process and partially used as placeholders in this script, and will not be actually loaded/trained
else:
    model = our_bert(
        config
    ).cuda()  # initialize from config, without using pretrained weights
...
optimizer = transformers.AdamW(
    model.parameters(), lr=our_config["lr"], weight_decay=our_config["wd"]
)
...
for e in range(1, our_config["training_epoch"] + 1):
    ...
    for r in range(
        int(len(train) / batchsize)
    ):  # no +1: in training, make sure the batchsize is stable
    ...
    if e % our_config["eval_epochs"] == 0:  # and our_config['mlm']==False:
        ...
        ALL_LOSS, ALL_PERIOD_LOSS, TIME, SEQ_LEN, IS_COVID_ADM = (
            pred_one_dataset_batch_causal(model, dataset=train, ignore_index=-100)
        )
        print("Now eval test set")
        ALL_LOSS, ALL_PERIOD_LOSS, TIME, SEQ_LEN, IS_COVID_ADM = (
            pred_one_dataset_batch_causal(model, dataset=test, ignore_index=-100)
        )
    ...
    if e % our_config["save_epochs"] == 0:
        ...
        for ignore_index in [-100]:
            ...
            ALL_LOSS, ALL_PERIOD_LOSS, TIME, SEQ_LEN, IS_COVID_ADM = (
                pred_one_dataset_batch_causal(
                    model, dataset=data, ignore_index=ignore_index
                )
            )
...
test_500_days = [
    ...
    if difference_time_A_B_seconds(
        timeA=i["Time"], timeB="01/01/2019 00:00", format_pattern="%m/%d/%Y %H:%M"
    )
    > 0
...
if our_config["mode"] == "pred":
    for model_ep in [100]:
        ...
        model = our_bert.from_pretrained(
            "ckpt/anomaly_BMC/anomaly_ep" + str(model_ep) + "/"
        ).cuda()
        model.eval()
        ...
        for ignore_index in [-100]:
            ALL_LOSS, ALL_PERIOD_LOSS, TIME, SEQ_LEN, IS_COVID_ADM = (
                pred_one_dataset_batch_causal(
                    model, dataset=test_500_days, ignore_index=ignore_index
                )
            )
